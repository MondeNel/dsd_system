...
export default function CommentsView({ entries }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      {/* Entry list – larger touch targets, bigger text */}
      <div className="col-span-1 glass-card rounded-2xl p-5 max-h-80 lg:max-h-none overflow-y-auto">
        <h4 className="mb-4 text-base font-semibold text-slate-800">All entries</h4>
        {entries.length === 0 && (
      ...
      {/* Comments panel – more padding, larger text */}
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto max-h-72 lg:max-h-none">
              ...
              {(selectedEntry.comments || []).map((c) => (
                <div
                  key={c.id}
                  className="rounded-xl border border-white/20 bg-white/40 p-4"
                >
            ...
            <div className="mt-5 flex gap-3">
              <input
                ...
                className="flex-1 rounded-xl border border-slate-200 bg-white/60 backdrop-blur px-4 py-2.5 text-sm placeholder-slate-400 focus:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-indigo-400/20"
              />
              <button
