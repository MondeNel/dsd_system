...
import { MessageCircle, X, Sparkles, ChevronRight, ChevronLeft, Check } from 'lucide-react';
...
export default function ChatAssistant({ onFillForm, onOpenForm }) {
  ...
  const handleNext = () => {
    if (step === 0 && !indicator) return;
    if (step === 1 && totalParticipants === 0) return;
    if (step === 2 && ageTotal !== totalParticipants) return;
    setStep((s) => s + 1);
  };
  ...
  const handleServiceToggle = (name) => {
    setServices((prev) => ({
      ...prev,
      [name]: prev[name] > 0 ? 0 : 1,
    }));
  };

  const handleServiceChange = (name, val) => {
    setServices((prev) => ({ ...prev, [name]: Math.max(0, val) }));
  };

  const canGoNext = () => {
    if (step === 0) return !!indicator;
    if (step === 1) return totalParticipants > 0;
    if (step === 2) return ageTotal === totalParticipants;
    return true;
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md glass-card rounded-2xl shadow-2xl overflow-hidden">
              ...
              {STEPS.map((_, i) => (
                <div
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    i <= step ? 'bg-indigo-500' : 'bg-slate-300'
                  } ${i <= step ? 'w-6' : 'w-4'}`}
                />
              ))}
            </div>
            ...
            <div className="px-5 py-4 max-h-[60vh] overflow-y-auto">
                      ...
                      return (
                        ...
                        >
                          ...
                          {selected && (
                            <StepperInput
                              value={services[svc]}
                              onChange={(val) => handleServiceChange(svc, val)}
                            />
                          )}
                        </div>
                      );
              ...
              {step === 5 && (
                <div>
                  <p className="text-sm font-medium text-slate-700 mb-3">Review entry</p>
